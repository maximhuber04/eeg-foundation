...
if load_mode != 1:
    train_index = train_index[:200] + train_index[-200:]
...
if load_mode == 0 or load_mode == 2:
    ...
    with open("/home/maxihuber/eeg-foundation/runs/eps_trainchns.json", "w") as f:
        json.dump(list(train_channels.values()), f, indent=4)
    with open("/home/maxihuber/eeg-foundation/runs/eps_testchns.json", "w") as f:
        json.dump(list(test_channels.values()), f, indent=4)
elif load_mode == 1:
    train_data, train_outputs, train_sr, train_dur, train_channels, train_datasets = (
        load_file_data(train_index, task_channels)
    )
    ...
    def forward(self, full_x):
        ...
        for win_size, x_emb in x_embeds.items():
            freqs_cis = select_freqs_cis(
                self.encoder,
                self.encoder.encoder_freqs_cis,
                H_W[win_size][0],
                H_W[win_size][1],
                win_size,
                x_emb.device,
            )
            x_emb = self.finetune_time_transformer(
                x_emb, freqs_cis=freqs_cis, nr_meta_tokens=nr_meta_patches
            )
...
checkpoint_callback = ModelCheckpoint(
    dirpath=f"/itet-stor/maxihuber/net_scratch/finetune_ckpts/{task_name}",
